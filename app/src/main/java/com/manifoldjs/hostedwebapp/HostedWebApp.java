...
public class HostedWebApp extends CordovaPlugin {
	...
	private static final String OFFLINE_PAGE = "index.html";
				...
				if (item != null) {
					...
					if (!source.isEmpty()) {
						...
						if (this.isMatchingRuleForPage(pageUrl, item, false)) {
							injectScripts(Arrays.asList(source), null);
						}
					}
				}
			}
			...
			public void run() {
				...
				me.activity.runOnUiThread(new Runnable() {
					...
					public void run() {
						...
						try {
							Method evaluateJavaScriptMethod = webView.getClass().getMethod("evaluateJavascript", String.class, ValueCallback.class);
